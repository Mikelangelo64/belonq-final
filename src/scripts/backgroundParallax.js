...
import randomInteger from './config/random';

function backgroundParallax(window) {
    ...
    for (let i = 0, len = this.options.pieces; i < len; ++i) {
      ...
      segPiecesHTML += clipPath
        ? `<div class="segmenter__piece" style="background-image: url(${
            this.imgsrc
          }); -webkit-clip-path: polygon(${left}% ${top}%, ${
            left + width
          }% ${top}%, ${left + width}% ${top + height}%, ${left}% ${
            top + height
          }%); clip-path: polygon(${left}% ${top}%, ${left + width}% ${top}%, ${
            left + width
          }% ${top + height}%, ${left}% ${top + height}%)"></div>`
  ...
  Segmenter.prototype._renderPiece = function (piece) {
    ...
    if (self.options.animation.translateZ != undefined) {
      if (typeof self.options.animation.translateZ === 'object') {
        ...
        piece.style.transform =
          piece.style.WebkitTransform = `translateZ(${randArr[idx]}px)`;
      } else {
        piece.style.transform =
          piece.style.WebkitTransform = `translateZ(${self.options.animation.translateZ}px)`;
      }
    }
    if (self.options.animation.translateY != undefined) {
      if (typeof self.options.animation.translateY === 'object') {
        ...
        piece.style.transform =
          piece.style.WebkitTransform = `translateY(${randArr[idx]}px)`;
      } else {
        piece.style.transform =
          piece.style.WebkitTransform = `translateY(${self.options.animation.translateY}px)`;
      }
    }
    if (self.options.animation.translateX != undefined) {
      if (typeof self.options.animation.translateX === 'object') {
        ...
        piece.style.transform =
          piece.style.WebkitTransform = `translateX(${randArr[idx]}px)`;
      } else {
        piece.style.transform =
          piece.style.WebkitTransform = `translateX(${self.options.animation.translateX}px)`;
      }
    }
    ...
    if (self.options.shadows && isObjEmpty(self.options.shadowsAnimation)) {
      ...
      shadowEl.style.opacity =
        self.options.shadowsAnimation.opacity != undefined
  ...
  Segmenter.prototype.animate = function () {
    ...
    if (this.options.animation.translateZ != undefined) {
      ...
      animProps.translateZ =
        typeof this.options.animation.translateZ === 'object'
          ? function (el, index) {
    ...
    if (this.options.animation.translateX != undefined) {
      animProps.translateX =
        typeof this.options.animation.translateX === 'object'
          ? function (el, index) {
              return anime.random(
                self.options.animation.translateX.min,
                self.options.animation.translateX.max
    ...
    if (this.options.animation.translateY != undefined) {
      animProps.translateY =
        typeof this.options.animation.translateY === 'object'
          ? function (el, index) {
              return anime.random(
                self.options.animation.translateY.min,
                self.options.animation.translateY.max
    ...
    this.debounceResize = debounce(function (ev) {
      ...
      self.pieces.forEach(function (piece, position) {
        ...
        if (self.options.positions === 'random') {
          ...
          clipTop =
            (self.options.positions[pos].top / 100) * self.dimensions.height;
          clipLeft =
            (self.options.positions[pos].left / 100) * self.dimensions.width;
          clipRight =
            (self.options.positions[pos].width / 100) * self.dimensions.width +
            clipLeft;
          clipBottom =
            (self.options.positions[pos].height / 100) *
      ...
      this.mousemove = function (ev) {
        ...
        requestAnimationFrame(function () {
          self.pieces.forEach(function (piece) {
            ...
            piece.parentNode.style.transform =
              piece.parentNode.style.WebkitTransform = `translate3d(${transX}px,${transY}px,0)`;
          });
        });
      };
...
export const backgroundParallaxInit = () => {
  if (!window.Segmenter) {
    return;
  }

  const segmenter = new Segmenter(
    document.querySelector('.background__image'),
    {
      pieces: 7,
      animation: {
        duration: 1500,
      ...
      parallax: true,
      parallaxMovement: { min: 20, max: 50 },
      positions: [
        {
          top: 10,
          left: randomInteger(0 + 20, 50 + 20),
          width: 20,
          height: 30,
        },
        { top: 8, left: randomInteger(0 + 35, 50 + 35), width: 30, height: 20 },
        {
          top: 25,
          left: randomInteger(-50 + 18, 50 + 18),
          width: 14,
          height: 25,
        },
        {
          top: 23,
          left: randomInteger(-50 + 50, 50 + 50),
          width: 20,
          height: 10,
        },
        {
          top: 30,
          left: randomInteger(-50 + 65, 50 + 65),
          width: 10,
          height: 30,
        },
        {
        ...
        {
          top: 50,
          left: randomInteger(-50 + 67, 50 + 67),
          width: 10,
          height: 20,
        },
      ],
